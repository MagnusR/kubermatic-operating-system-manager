...
import (
	...
	mcbootstrap "github.com/magnusr/kubermatic-machine-controller/pkg/bootstrap"
...
func TestDefaultCloudConfigGenerator_Generate(t *testing.T) {
	bootstrapConfig := mcbootstrap.BootstrapCloudConfig
	...
	testCases := []struct {
		...
		secretType          *mcbootstrap.CloudConfigSecret
...
`),
		...
		{
			name:       "generated bootstrap cloud-init for ubuntu",
			secretType: &bootstrapConfig,
			osc: &osmv1alpha1.OperatingSystemConfig{
				Spec: osmv1alpha1.OperatingSystemConfigSpec{
					OSName:    "ubuntu",
